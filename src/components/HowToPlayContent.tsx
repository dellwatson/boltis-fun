...
import { generateRandomCards } from "../utils/randomCards";
...
const HowToPlayContent: React.FC<HowToPlayContentProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "basics" | "cards" | "modes" | "scoring"
  >("basics");

  // State for random cards with auto-refresh
  const [randomCards, setRandomCards] = useState<CardType[]>([]);

  // Auto-refresh cards with a slower timing
  React.useEffect(() => {
    const timer = setInterval(() => {
      setRandomCards(generateRandomCards(8));
    }, 8000); // Slightly faster refresh (8 seconds)

    // Initial set with a small delay
  ...
  const handleTabChange = (tab: "basics" | "cards" | "modes" | "scoring") => {
    setActiveTab(tab);
  };
  ...
  return (
    <div
      className={`bg-white/10 backdrop-blur-sm rounded-3xl border border-white/20 overflow-hidden ${className}`}
    >
      {/* Header */}
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-white/80 text-xl">
              Learn the rules and dominate the arena
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-1 bg-white/10 rounded-lg p-1">
          {[
            ...
            <motion.button
              ...
              onClick={() =>
                handleTabChange(
                  tab.id as "basics" | "cards" | "modes" | "scoring",
                )
              }
          ...
          {activeTab === "basics" && (
            ...
            >
              ...
              <p className="text-xl text-white/90 mb-8 leading-relaxed">
                Be the first to empty your hand or have the lowest score when
                time runs out. Match colors, numbers, or use special cards to
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="text-center p-6 bg-green-500/20 rounded-2xl border border-green-400/30">
                    ...
                    <p className="text-white/80">
                      First to play all cards wins instantly
                    </p>
                  </div>
                  ...
                  <div className="text-center p-6 bg-yellow-500/20 rounded-2xl border border-yellow-400/30">
                    ...
                    <p className="text-white/80">
                      When time runs out, lowest points wins
                    </p>
                  </div>
                  ...
                  <div className="text-center p-6 bg-red-500/20 rounded-2xl border border-red-400/30">
                    ...
                    <p className="text-white/80">
                      Last player standing in time-bank mode
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="b border-blue-400/30 mb-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
                  <div className="flex flex-col items-center md:pt-10">
                    ...
                    <div className="mt-4 text-center">
                      <h5 className="font-bold text-blue-300">Draw Pile</h5>
                      <p className="text-white/80 text-sm">
                        Draw cards from here when you can't play or are forced
                        to draw.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-center">
                      ...
                      >
                        <AnimatePresence mode="wait">
                          {randomCards.slice(0, 8).map((card, index) => {
                            if (card.type === "void") return null;

                            // Fixed card dimensions
                            const cardWidth = 120;
                            const cardHeight = 180;

                            // Calculate staggered offsets for natural stacking
                            const stackOffset = 2; // pixels offset between cards
                            const maxOffset = 12; // max pixels cards can be offset in the stack
                            const xOffset =
                              Math.min(index * stackOffset, maxOffset) -
                              maxOffset / 2;
                            const yOffset =
                              Math.min(index * stackOffset, maxOffset) -
                              maxOffset / 2;
                            const rotation = -3 + index * 0.5; // Slight rotation that increases with stack

                            // Start positions (simulating from player's hand)
                            const startX = -100 + index * 30;
                            const startY = 100 - index * 5;

                            // End positions (slightly randomized within stack area)
                            const endX = xOffset + (Math.random() * 10 - 5);
                            const endY = yOffset + (Math.random() * 10 - 5);
                            const endRotate =
                            ...
                            return (
                              <motion.div
                                ...
                                animate={{
                                  ...
                                  opacity: 1,
                                  transition: {
                                    delay: 0.2 + index * 0.15, // Quicker succession
                                    type: "spring",
                                    damping: 20,
                                    stiffness: 100,
                                    mass: 0.7,
                              ...
                              >
                                <div className="w-full h-full">
                                  <Card
                                    ...
                                    size="medium"
                                    animate={true}
                                    className="w-full h-full shadow-lg transition-all duration-300"
                                  />
                                </div>
                              </motion.div>
                            );
                    ...
                    <div className="mt-4 text-center">
                      ...
                      <p className="text-white/80 text-sm">
                        Play cards by matching the top card's color, number, or
                        element.
                      </p>
                    </div>
                  </div>
                ...
                {/* <div className="mb-6 p-4 bg-white/10 rounded-xl border border-white/20">
                  <p className="text-white/90 text-center">
                    ...
                    <span className="text-white/70">
                      {" "}
                      Future expansions may include Wind, Rock, Ice, and more
            ...
            >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  ...
                  <p className="text-white/80">
                    Match by color, number, or element. Each element has cards
                    numbered 1-9.
                  </p>
                </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-4 p-4 bg-green-500/20 rounded-xl border border-green-400/30">
                      ...
                      <div>
                        ...
                        <p className="text-white/80 text-sm">
                          Removes matching cards and weak elements from discard
                          pile
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 p-4 bg-red-500/20 rounded-xl border border-red-400/30">
                      ...
                      <div>
                        ...
                        <p className="text-white/80 text-sm">
                          Next player draw cards or counter it with stronger
                          element.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 p-4 bg-gray-500/20 rounded-xl border border-gray-400/30">
                      ...
                      <div>
                        ...
                        <p className="text-white/80 text-sm">
                          Transfers discard pile to next player and changes
                          color
                        </p>
                      </div>
                    </div>
              ...
              <div className="bg-gradient-to-br from-yellow-500/20 to-orange-500/20 p-6 rounded-2xl border border-yellow-400/30">
                <h4 className="text-xl font-bold text-white mb-4">
                  🚀 Additional Card Types (Coming Soon)
                </h4>
                <div className="space-y-3 text-white/90">
                  <div>
                    <strong>Locked Element Card:</strong> Locks the current
                    element for several turns
                  </div>
                  <div>
                    <strong>Mirror Card:</strong> Copies the effect of the last
                    played special card
                  </div>
                  <div>
                    <strong>Remove Stack:</strong> Removes all cards of a
                    specific type from the discard pile
                  </div>
                  <div>
                    <strong>Bomb-Time:</strong> Having bomb card to play fast
                    before explosion
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-blue-500/20 p-6 rounded-2xl border border-blue-400/30">
                  ...
                  <p className="text-white/80 mb-4">
                    Traditional gameplay with turn timers
                  </p>
                ...
                <div className="bg-purple-500/20 p-6 rounded-2xl border border-purple-400/30">
                  ...
                  <p className="text-white/80 mb-4">
                    Personal time bank with elimination
                  </p>
                  <ul className="text-sm text-white/70 space-y-1">
                ...
                <div className="bg-red-500/20 p-6 rounded-2xl border border-red-400/30">
                  ...
                  <p className="text-white/80 mb-4">
                    Fast-paced with shorter timers
                  </p>
                  <ul className="text-sm text-white/70 space-y-1">
            ...
            >
              ...
              <div className="text-center bg-green-500/20 p-8 rounded-2xl border border-green-400/30">
                ...
                <p className="text-xl text-white/90">
                  Empty your hand first or have the fewest points when time runs
                  out
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-purple-500/20 p-6 rounded-xl border border-purple-400/30">
                  <h5 className="text-lg font-bold text-purple-400 mb-3">
                    Penalty System
                  </h5>
                  <ul className="space-y-2 text-white/90">
