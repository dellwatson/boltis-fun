...
import { X, Timer, Eye, Settings, Layers, Clock, Zap, Palette, Shuffle, Plus, User, Bot, Car as Cards, Maximize2, Minimize2, RotateCcw } from 'lucide-react';
...
const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'game' | 'cards' | 'display'>('game');
  ...
  const toggleLandscapeMode = () => {
    const currentOrientation = screen.orientation?.type || 'portrait-primary';
    ...
    if (currentOrientation.includes('portrait')) {
      ...
      if (screen.orientation?.lock) {
        screen.orientation.lock('landscape-primary').catch((error) => {
          ...
          // Fallback: suggest manual rotation
          alert('Please rotate your device to landscape mode for the best gaming experience!');
        });
      } else {
        alert('Please rotate your device to landscape mode for the best gaming experience!');
      }
    } else {
      // Try to unlock or go back to portrait
                  ...
                  <div className={`space-y-8 ${isFullscreen ? 'max-w-none' : ''}`}>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="text-lg font-semibold text-gray-800">Mobile Controls</h3>
                      </div>
                      
                      <motion.button
                        onClick={toggleLandscapeMode}
                        className="w-full flex items-center justify-between p-4 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors border border-blue-200"
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-left">
                            <span className="font-medium text-blue-800">Toggle Landscape Mode</span>
                            <p className="text-sm text-blue-600">Rotate to landscape for better gameplay</p>
                          </div>
                        </div>
                      ...
                      <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
                        <h4 className="font-medium text-yellow-800 mb-2">📱 Mobile Tips</h4>
                        <ul className="text-sm text-yellow-700 space-y-1">
                          <li>• Landscape mode provides better card visibility</li>
                          <li>• Use fullscreen mode for immersive gameplay</li>
                          <li>• Install as PWA for native app experience</li>
                          <li>• Pinch to zoom if cards appear too small</li>
                        </ul>
                      </div>
                    </div>
                  </div>
